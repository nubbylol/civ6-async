-- ===========================================================================
--	Action Panel  (modded by civ6-async)
-- ===========================================================================

print("civ6-async: ActionPanel.lua replacement loaded");
...
function CheckAutoEndTurn( eCurrentEndTurnBlockingType:number )
	local pPlayer = Players[Game.GetLocalPlayer()];
	if pPlayer ~= nil then
		-- civ6-async: two engine gates removed:
		--   `not UI.SkipNextAutoEndTurn()` — sticky flag set after diplomatic
		--     greetings / wonder discoveries / ruin rewards; stays true after
		--     popup dismissal and silently blocks auto-end.
		--   `UserConfiguration.IsAutoEndTurn()` — the engine returns false for
		--     this option in any IsAnyMultiplayer() mode (hotseat included),
		--     overriding our ForceAutoEndTurn. We want auto-end in hotseat
		--     specifically, so the option-toggle gate has to go.
		-- Every other gate (ready unit, city ranged attack, turn-complete,
		-- unreadied, expansion content) is preserved.
		-- civ6-async: explicit ready-unit check. The base game's
		-- CheckUnitsHaveMovesState() is itself gated by IsAutoEndTurn(),
		-- which is forced false by the engine in MP/hotseat — so without
		-- our own check, a turn-1 settler with moves would be skipped.
		local hasReadyUnit = pPlayer:GetUnits():GetFirstReadyUnit() ~= nil;

		if not m_unreadiedTurn
			and eCurrentEndTurnBlockingType == EndTurnBlockingTypes.NO_ENDTURN_BLOCKING
			and not hasReadyUnit
			and not CheckCityRangeAttackState()
...
function OnNotificationDismissed( playerID:number, notificationID:number )
	if playerID == Game.GetLocalPlayer() then
		...
		-- ============================================================
		-- civ6-async: re-check auto-end-turn on ANY notification
		-- dismissal, not just city-range. CheckAutoEndTurn (patched
		-- above to ignore SkipNextAutoEndTurn) handles all gating.
		-- ============================================================
		if not wasCityRangeNotification then
			CheckAutoEndTurn(NotificationManager.GetFirstEndTurnBlocking(Game.GetLocalPlayer()));
		end
	end
end
...
-- ===========================================================================
-- civ6-async: deferred auto-end-turn check
--
-- The naive approach (call CheckAutoEndTurn at LocalPlayerTurnBegin) hits
-- a timing race: the engine hasn't yet evaluated newly-spawned units or
-- just-completed productions, so GetFirstReadyUnit() returns nil and the
-- gate passes — we end the turn before the player gets to use a unit
-- that should have been theirs to move that turn.
--
-- Strategy:
--   * Primary trigger is PlayerTurnActivated, which the engine fires AFTER
--     fully activating the player (units settled, productions reflected).
--   * Belt-and-braces retry on each GameCoreEventPublishComplete tick.
--   * Disarm immediately on UnitAddedToMap for the local player — a new
--     unit is unambiguous evidence the player has work to do.
--   * Disarm on LocalPlayerTurnEnd so we never fire on a stale arming.
-- ===========================================================================
-- Settle delay: ticks of GameCoreEventPublishComplete to *wait* before our
-- first check. Gives the engine room to fire UnitAddedToMap, populate
-- production-completed units, etc. Without this we'd try-and-end before
-- the engine gets a chance to expose the new state to us.
local CIV6_ASYNC_SETTLE_TICKS = 12
local CIV6_ASYNC_TRY_TICKS    = 30

local civ6Async_settleLeft = 0
local civ6Async_tryLeft    = 0

function civ6Async_armAutoEndCheck()
	civ6Async_settleLeft = CIV6_ASYNC_SETTLE_TICKS
	civ6Async_tryLeft    = CIV6_ASYNC_TRY_TICKS
end

local function civ6Async_disarmAutoEndCheck()
	civ6Async_settleLeft = 0
	civ6Async_tryLeft    = 0
end

local function civ6Async_tryEndIfClean()
	local pPlayer = Players[Game.GetLocalPlayer()]
	if pPlayer ~= nil and pPlayer:GetUnits():GetFirstReadyUnit() ~= nil then
		civ6Async_disarmAutoEndCheck()
		return
	end
	local blockingType = NotificationManager.GetFirstEndTurnBlocking(Game.GetLocalPlayer())
	CheckAutoEndTurn(blockingType)
end

local function civ6Async_onTick()
	if civ6Async_settleLeft > 0 then
		civ6Async_settleLeft = civ6Async_settleLeft - 1
		return
	end
	if civ6Async_tryLeft <= 0 then return end
	civ6Async_tryLeft = civ6Async_tryLeft - 1
	civ6Async_tryEndIfClean()
end

local function civ6Async_onPlayerTurnActivated(playerID, bFirstTime)
	if playerID == Game.GetLocalPlayer() then
		civ6Async_armAutoEndCheck()
		-- No immediate try — let the settle delay run first so
		-- UnitAddedToMap and other engine events get a chance to fire.
	end
end

local function civ6Async_onUnitAddedToMap(playerID, unitID)
	if playerID == Game.GetLocalPlayer() then
		civ6Async_disarmAutoEndCheck()
	end
end

Events.PlayerTurnActivated.Add(civ6Async_onPlayerTurnActivated)
Events.UnitAddedToMap.Add(civ6Async_onUnitAddedToMap)
Events.GameCoreEventPublishComplete.Add(civ6Async_onTick)
Events.LocalPlayerTurnEnd.Add(civ6Async_disarmAutoEndCheck)
